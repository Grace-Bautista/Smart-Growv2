...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  @override void dispose() { _commands.removeListener(_rebuild); super.dispose(); }
  void _rebuild() { if (mounted) setState(() {}); }
  @override void initState() { super.initState(); _commands.addListener(_rebuild); _commands.start(); }

  List<Sensor> _sensorsFrom(SensorData data) {
    String reading(double? value, SensorReadingStatus status, int decimals) =>
        status.isFresh(DateTime.now()) && value != null ? value.toStringAsFixed(decimals) : '--';
    return [
      Sensor(label: 'Humidity', value: reading(data.humidity, data.humidityStatus, 0), unit: '%', icon: Icons.water_drop_outlined, hasInfo: true),
      Sensor(label: 'Temp', value: reading(data.environmentTemperature, data.environmentTempStatus, 1), unit: 'c', icon: Icons.thermostat_outlined, hasInfo: true),
      Sensor(label: 'CO2', value: reading(data.co2, data.co2Status, 0), unit: 'ppm', icon: Icons.cloud_outlined, hasInfo: true),
    ];
  }
  void _showSnack(String message) => ScaffoldMessenger.of(context)..hideCurrentSnackBar()..showSnackBar(SnackBar(content: Text(message), duration: const Duration(seconds: 2), behavior: SnackBarBehavior.floating));
  Future<void> _send(IotCommandTarget target, Object value) async {
    try { await _commands.submit(target, value); } catch (error) { if (mounted) _showSnack('Command unavailable: $error'); }
  }
  ...
  @override Widget build(BuildContext context) => StreamBuilder<SensorData>(
    ...
    builder: (context, snapshot) {
      ...
      return Scaffold(
        body: CustomScrollView(slivers: [_header(context), SliverToBoxAdapter(child: _body(context, data, available, snapshot.hasError))]),
        bottomNavigationBar: SmartGrowBottomNav(selectedIndex: _navIndex, onTap: (i) => setState(() => _navIndex = i), onPowerTap: () => _showSnack(available ? 'Receiving live RTDB data' : 'ESP32 heartbeat is stale')),
      );
    },
  );
  ...
  RefillMode _refillMode(SensorData data) => data.refillPumpMode == AutomationMode.automatic ? RefillMode.auto : (data.refillPumpOn ?? false) ? RefillMode.on : RefillMode.off;
  Future<void> _setRefillPump(SensorData data, RefillMode mode) => switch (mode) { RefillMode.auto => _send(IotCommandTarget.refillPumpMode, 'automatic'), RefillMode.on => _send(IotCommandTarget.refillPump, true), RefillMode.off => _send(IotCommandTarget.refillPump, false) };
  Widget _deviceRow(SensorData data, bool enabled) => Row(children: [Expanded(child: DeviceSwitchCard(title: 'UV Light', description: 'ESP32-reported controller output.', icon: Icons.wb_sunny_outlined, value: data.uvLightOn ?? false, enabled: enabled && !_pending(IotCommandTarget.uvLight), onChanged: (v) => _send(IotCommandTarget.uvLight, v))), const SizedBox(width: AppTheme.space3), Expanded(child: DeviceSwitchCard(title: 'Ventilation', description: 'ESP32-reported vent-fan controller output.', icon: Icons.air_rounded, value: data.ventFanOn ?? false, enabled: enabled && !_pending(IotCommandTarget.ventFan), onChanged: (v) => _send(IotCommandTarget.ventFan, v)))]);
}
